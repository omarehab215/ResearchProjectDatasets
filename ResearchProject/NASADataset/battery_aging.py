...
if view == 'Data Analysis':
    ...
    for key, val in cycle_opt.items():
        ...
        ls = '-' if key == 0 else '--'
        ax[0].plot('Time', 'Voltage_charge', data=df[mask0 & mask1], color=palette[bat_opt], ls=ls,
                   label=f'Cycle {val}')
        ax[1].plot('Time', 'Current_charge', data=df[mask0 & mask1], color=palette[bat_opt], ls=ls,
                   label=f'Cycle {val}')
    plt.legend()
